...
const CheckoutForm = () => {
	...
	const handleSubmit = async (ev) => {
		...
		let userDetails = {
			...
			mobile: ev.target.phone.value,
			address: ev.target.address.value,
			pin: ev.target.pin.value,
		};
		let key = myUser.nickname;
		setMyUser({ ...userDetails });
		localStorage.setItem(key, JSON.stringify(userDetails));

		localStorage.setItem("phone", ev.target.phone.value);
		localStorage.setItem("address", ev.target.address.value);
		localStorage.setItem("pin", ev.target.pin.value);
